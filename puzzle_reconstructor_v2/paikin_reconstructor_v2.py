...
Advanced puzzle reconstruction using Best Buddies algorithm
...
class PaikinSolverV2(PuzzleSolverBaseV2):
    ...
    def extract_features_rotated(self, img: np.ndarray, rotation: int) -> Dict[str, np.ndarray]:
        ...
        # LAB color space normalized [0,1]
        lab = cv2.cvtColor(rotated, cv2.COLOR_BGR2LAB).astype(np.float32) / 255.0
        
        # Gradient magnitude (Scharr) normalized [0,1]
    ...
    def calculate_cost(self, idx_a: int, idx_b: int, direction: str,
        ...
        """
        Calculates compatibility cost between two pieces with rotations.
        Uses dynamic weighting based on content type.
        """
        ...
        # Measure "Activity" using magnitude channel (index 3)
        ...
        if is_bg_a and is_bg_b:
            # Both are background: only color matters
            cost = diff_color * 2.0
        elif is_bg_a != is_bg_b:
            # Mixed: high penalty
            cost = 5.0 + diff_color + diff_grad
        else:
            # Both are objects: weighted combination
    ...
    def solve(self):
        ...
        print(f"\n{'='*60}")
        print(f"PAIKIN RECONSTRUCTOR V2 - With Rotation Support")
        print(f"{'='*60}")
        print(f"Pieces: {n_slices} ({rows}x{cols})")
        print(f"Features: LAB + Gradient Magnitude")
