#!/usr/bin/env python3
"""
Generate Clean Grid Visualizations

Creates puzzle reconstructions with only the grid overlay (no metrics text)
for color, gradient, paikin, and random methods.
"""
...
def read_reconstruction_map(map_file: str) -> dict:
    ...
    Returns:
        Dictionary with reconstruction info: {(row, col): filename}
    """
    reconstruction = {}
    rows = 0
    cols = 0
    ...
    # Find the position mapping section (support both English and Spanish)
    in_mapping = False
    for line in lines:
        # Check for header lines (English or Spanish)
        if "POSITION | ORIGINAL FILE" in line or "POSICIÓN | ARCHIVO ORIGINAL" in line:
            in_mapping = True
            continue
        
        if in_mapping and "(" in line and ")" in line and "->" in line:
            # Parse line like: (0,0) -> pinguino_slice_000.png
            ...
            if len(parts) == 2:
                ...
                reconstruction[(row, col)] = filename
                rows = max(rows, row + 1)
                cols = max(cols, col + 1)
...
def create_clean_grid(sliced_dir: str, reconstruction_map: dict, rows: int, cols: int, 
    ...
    Args:
        ...
        reconstruction_map: Dictionary mapping (row, col) to filename
        rows: Number of rows
        cols: Number of columns
        output_path: Path to save the clean grid image
    """
    # Load first image to get dimensions
    first_file = reconstruction_map[(0, 0)]
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            filename = reconstruction_map[(r, c)]
...
def main():
    ...
    SLICED_DIR = os.path.join("sliced_images_v1", f"{IMAGE_NAME}_{NUM_SLICES}slices")
    ...
    for method in methods:
        ...
        # Read reconstruction map
        map_file = os.path.join(BASE_DIR, f"{method}_reconstruction_map.txt")
